...
public class OpenAiService {
    ...
    public AiResponse getBudgetTips(BudgetRequest req) {
        ...
        String incomePositionText = statService.getIncomePosition(
        ...
        Map<String, Object> body = Map.of(
                ...
                "input", List.of(
                        Map.of(
                                "role", "system",
                                "content", """
                                    Du er en hjælpsom dansk økonomi-assistent.
                                    Du giver konkrete, realistiske budgetråd til studerende i Danmark.
                                    Du spørger IKKE om flere spørgsmål tilbage – du laver bare et bud på et budget.
                                    Svar altid på dansk, brug gerne punktopstillinger.
                                    """
                        ),
                        Map.of(
                                "role", "user",
                                "content", buildUserPrompt(req, incomePositionText)
                        )
                )
        );
        ...
        try {
            ...
            return new AiResponse(text, incomePositionText);

        } catch (Exception e) {
            logger.error("Fejl ved kald til OpenAI", e);
            return new AiResponse("Der opstod en fejl ved kald til OpenAI. Tjek backend-loggen mens du udvikler.",
                    null);
        }
    }
